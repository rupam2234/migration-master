...
export async function GET(req: NextRequest) {
    ...
    try {
        ...
        const assetResults = await fetchItems(resource, projectName, 50, token);
...
/**
     * to check exposed end point ---
     * (Invoke-WebRequest -Uri "https://staging.orgoneenergy.org/wp-json/" -UseBasicParsing).Content | ConvertFrom-Json | Select-Object -ExpandProperty namespaces
     * 
     * Check for expose of migration-master/v1
     */
async function fetchItems(resource: WordPressResource, domain: string, ITEM_PER_PAGE: number = 50, token: string) {
    ...
    const buildUrl = (page: number): string => {
        ...
        switch (resource) {
            ...
            case "customers":
            case "coupons":
                return `${base}/wc/v3/${resource}?page=${page}&per_page=${ITEM_PER_PAGE}`;
            default:
